...
export default function Profile() {
    ...
    const projectList = Object.keys(projects).length > 0 ?
        <div className={"main-task-menu vertical-menu"}>{Object.keys(projects).map((key, index) => {
            const id = key;
            ...
            return (<TrayItem key={id} id={id} name={name} description={description} path={'/create/project/'}
                              onClick={(evt) => {
    ...
    const eventList = Object.keys(events).length > 0 ?
        <div className={"main-task-menu vertical-menu"}>{Object.keys(events).map((key, index) => {
            const id = key;
            ...
            return (<TrayItem key={id} id={id} name={name} description={description} path={'/create/event/'}
                              onClick={(evt) => {
